...
int prPrimitiveErrorString(struct VMGlobals* g, int numArgsPushed) {
    ...
    case errFailed:
        str = "Failed.";
        break;
    case errBadPrimitive:
        str = "Bad Primitive.";
        break;
    case errWrongType:
        str = "Wrong type.";
        break;
    case errIndexNotAnInteger:
        str = "Index not an Integer";
        break;
    case errIndexOutOfRange:
        str = "Index out of range.";
        break;
    case errImmutableObject:
        str = "Attempted write to immutable object.";
        break;
    case errNotAnIndexableObject:
        str = "Not an indexable object.";
        break;
    ...
    case errOutOfMemory:
        ...
        break;
    case errCantCallOS:
        str = "Operation cannot be called from this Process. Try using AppClock instead of SystemClock.";
        break;
    case errException: {
        ...
        if (lastPrimitiveException) {
            try {
                ...
                str = std::string("caught exception \'") + errorString + "\' in primitive in method "
                    + lastPrimitiveExceptionClass + ":" + lastPrimitiveExceptionMethod;
                break;
            }
        } else {
            str = std::string("caught unknown exception in primitive in method ") + lastPrimitiveExceptionClass + ":"
                + lastPrimitiveExceptionMethod;
            break;
        }
        break;
    }
...
int prFunctionDefDumpContexts(struct VMGlobals* g, int numArgsPushed) {
    ...
    while (slotRawBlock(a)) {
        post("%2d context %s %p\n", i++, slotRawSymbol(&slotRawObject(a)->classptr->name)->name,
             slotRawInt(&slotRawBlock(a)->contextDef));
...
int arrayPerformMsg(struct VMGlobals* g, int numArgsPushed) {
    ...
    pslot = arraySlot;
    qslot = selSlot;
    for (m = 0, mmax = array->size - 2; m < mmax; ++m)
        slotCopy(++pslot, ++qslot);
...
int prObjectCopySeries(struct VMGlobals* g, int numArgsPushed) {
    ...
    if (IsInt(c))
        ...
        second = first < last ? first + 1 : first - 1;
    else
        return errWrongType;
...
int prUGenCodeString(struct VMGlobals* g, int numArgsPushed) {
    ...
    if (err)
        return err;
    if (!isKindOfSlot(cc, class_array) && !isKindOfSlot(cc, class_symbolarray))
        return errWrongType;
    if (!isKindOfSlot(dd, class_array))
        return errWrongType;
    ...
    while ((c = *in++) != 0) {
        if (c == '@') {
            ...
            if (slotIndex >= 0) {
                ...
                if (!isKindOfSlot(inputStringSlot, class_string))
                    return errWrongType;
...
void switchToThread(VMGlobals* g, PyrThread* newthread, int oldstate, int* numArgsPushed) {
    ...
    *numArgsPushed = slotRawInt(&newthread->numArgsPushed);
    ...
    // slotRawObject(&slotRawThread(&g->process->mainThread)->stack));
...
int prThreadInit(struct VMGlobals* g, int numArgsPushed) {
    ...
    initPyrThread(g, thread, b, stacksize, (PyrInt32Array*)(slotRawObject(&g->thread->randData)), beats, seconds,
                  &g->thread->clock, true);
...
int prThreadSetRandData(struct VMGlobals* g, int numArgsPushed) {
    ...
    if (!isKindOfSlot(b, class_int32array))
        return errWrongType;
    if (slotRawObject(b)->size < 3)
        return errWrongType;
...
static int prLanguageConfig_getLibraryPaths(struct VMGlobals* g, int numArgsPushed, int pathType) {
    ...
    const DirVector& dirVector =
        (pathType == includePaths) ? gLanguageConfig->includedDirectories() : gLanguageConfig->excludedDirectories();
...
int nextPrimitiveIndex() { return gPrimitiveTable.size + 1; }
